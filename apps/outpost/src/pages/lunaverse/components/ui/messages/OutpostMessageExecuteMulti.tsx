...
import { useExecuteMessageCustom } from 'pages/lunaverse/hooks/useExecuteMessageCustom';
import {
    ...
    const { tx } = useExecuteMessageCustom(
        ...
        (address, contractAddress, exec_msg, coins) => {
            ...
            if (
                ...
                message.multiType ===
                    ContractExecuteMultiMessageTypes.TRANSFER_LUV
            ) {
                const msgs: Array<{ address: string; amount: string }> =
                    exec_msg;
                result = msgs.map((msg) => {
